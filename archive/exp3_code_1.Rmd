...
# Save
test_acc_2_no_invdis <- pair_comp(test_acc_2_train_cont %>%
                                   data.frame() %>% 
                                   dplyr::filter(match_type == "identity"), 
                                 "no training - invariant dissimilar")

# Save
test_acc_2_train_no <- test_acc_2_train_form %>% 
  dplyr::filter(cond_exp_fac == "no training" & match_type == "identity") %>%
  pull(report)
test_acc_2_train_invdis <- test_acc_2_train_form %>% 
  dplyr::filter(cond_exp_fac == "invariant dissimilar" & match_type == "identity") %>%
  pull(report)
```
...
```{r test_2_17}
# Filter and factor
dat_test_spk <- dat_test_rt %>%
  dplyr::filter(version == "2a" & cond_sim == "direct") %>%
  mutate(speaker_code = case_when(
    speaker %in% c(3,4) ~ "sim",
    speaker %in% c(5,6) ~ "diss"
  ),
  across(.cols = c("participant", "prime", "target", "match_type",
                   "cond_var", "speaker_code", "speaker"),
         as.factor),
  trial_cent = trial - mean(trial),
  vot_cent = vot - mean(vot),
  freq_prime_cent = FreqZipfUS_prime - mean(FreqZipfUS_prime),
...
# Add contrasts
contrasts(dat_test_spk$cond_var) <- contrast_var
contrasts(dat_test_spk$match_type) <- contrast_match_2
contrasts(dat_test_spk$speaker_code) <- contr.sum(2)
```

```{r test_2_18}
# Run
post_hoc <- lmer(inv_rt ~ speaker_code*match_type*cond_var +
                   trial_cent + freq_prime_cent:freq_target_cent +
                   vot_cent +
                   (1|prime:target) + (1|participant),
                 data = dat_test_spk)
...
# Model comparison
post_hoc_comp <- Anova(post_hoc, test = "Chi", type = "III")

# Get means
post_hoc_means_spk <- emmeans(post_hoc, ~ speaker_code*match_type*cond_var)

# Run contrasts
post_hoc_cont_spk <- pairs(post_hoc_means_spk, 
                           by = c("match_type", "cond_var"), adj = "none")
...
dat_survey <- read.csv("~/Mirror/dissertation/12_behavioral/data/quality/dat_survey.csv") %>%
  mutate(acc_l1 = if_else(spk_lang_1 == "Spanish", 1/2, -1/2),
         ...
         acc_subregion = if_else(spk_region == "Central", 1/2, -1/2),
...
dat_pro <- read.csv("~/Mirror/dissertation/12_behavioral/data/quality/dat_pro.csv") %>%
  select(participant, par_accent, par_age, par_accent_type, par_accent_strength) %>%
  mutate(par_accent_cc = if_else(par_accent == "Yes", 1/2, -1/2)) %>%
...
dat_corr <- dat_survey %>%
  ...
  mutate(train_cc = if_else(training == "with", 1/2, -1/2),
         test_cc = if_else(test_task == "match", -1/2, 1/2)) %>%
  select(acc_l1, acc_l2, acc_region, acc_accent,
         spk_ease, spk_percep, spk_accent_strength,
         spk_fluency_1, spk_fluency_2, test_cc, train_cc, 
         par_accent_strength, par_age)

# rt_competitor

# # Set column names
# colnames(dat_corr) <- c("L1: Spanish", "L2: English", "Region: Americas", "Accent: L2",
#                         "Ease", "Comprehensibility", "Accent strength",
#                         "L1 fluency", "L2 fluency", "Competitor RT", "With/out exposure",
#                         "Participant accent strength", "Participant age")
```
