...
```{r exp1-fig, fig.cap = "Experiment 1 design.", dpi = 300}
...
In addition, we included mean-centered prime frequency and mean-centered target frequency as an interaction term (since these variables were highly correlated).
...
For accuracy, there was a significant three-way interaction among Variability, Similarity, and Word type (`r exp_acc_1a_comp_3_form`), as well as a significant two-way interaction between Variability and Word type (`r exp_acc_1a_comp_2_form`).
To follow up on these interactions, we first conducted pairwise comparisons between levels of Similarity within each level of Variability and Word type.
Control-Variant exposure yielded higher accuracy on pseudowords (`r exp_acc_1a_mean_contnonvar`) than Indirect-Variant exposure (`r exp_acc_1a_mean_indnonvar`; `r exp_acc_1a_cont_var_non`).
No other contrasts were significant (*ps* > .05).
We then conducted pairwise comparisons between levels of Variability within each level of Similarity and Word type.
Control-Invariant exposure yielded higher accuracy on real words (`r exp_acc_1a_mean_contrealinvar`) than Control-Variant exposure (`r exp_acc_1a_mean_contrealvar`).
No other contrasts were significant (*ps* > .05).

For RTs, we observed a significant three-way interaction among Variability, Similarity, and Word type (`r exp_rt_1a_comp_3_form`).
The main effect of Similarity was also significant (`r exp_rt_1a_comp_1_form`).
...
We observed a main effect of Exposure on accuracy (`r test_acc_1a_train_exp_form`); however, pairwise comparisons (averaging across levels of Target) did not reveal significant differences between the Test-only condition and any of the exposure conditions (*ps* > .05).
To further investigate the main effect of Exposure, we conducted pairwise comparisons between each of the exposure conditions, but there were no significant differences (*ps* > .05).
...
In the exposure task, participants did not differ according to Similarity or Variability.
The Control groups displayed slower RTs than the Direct or Indirect groups; however, these differences likely reflect acoustic differences between the articulation of fricatives versus stops rather than meaningful differences in processing.
...
The exposure task did not benefit performance on the test task.
